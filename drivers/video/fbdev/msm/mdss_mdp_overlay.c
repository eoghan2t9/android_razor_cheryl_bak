	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	panel_min_fps = mdss_panel_get_min_framerate(&pdata->panel_info);
	if (pdata->panel_info.type == MIPI_CMD_PANEL && data.fps < panel_min_fps) {
		data.fps = panel_min_fps;
	} else if (pdata->panel_info.type == MIPI_VIDEO_PANEL && data.fps <= panel_min_fps) {
